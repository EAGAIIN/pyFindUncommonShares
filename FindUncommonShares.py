...
class MicrosoftDNS(object):
    ...
    def resolve(self, target_name):
        """
        Resolves a target name to a list of IP addresses.
...
def is_port_open(target, port):
    """
    Checks if a port is open on a target.
    When target_ip is provided (impacket smbclient.py style), we connect
    directly to the IP so this works correctly through proxychains.
    """
...
def init_smb_session(options, target_name, target_ip, domain, username, password, lmhash, nthash, port=445, debug=False):
    """
    Mirrors impacket smbclient.py pattern:
        SMBConnection(address, target_ip)
    where:
        address   = hostname/FQDN (used for NBT name and Kerberos SPN)
        target_ip = actual IP to connect to (bypasses DNS, works with proxychains)

    If target_ip is None (shouldn't happen in our flow), falls back to address.
    """
    connect_target = target_ip if target_ip is not None else target_name

    smbClient = SMBConnection(target_name, connect_target, sess_port=int(port))
...
def worker(options, target, domain, username, password, lmhash, nthash, results, lock):
    """
    Mirrors impacket smbclient.py pattern:
        - target_name = address (FQDN or hostname, passed as first arg to SMBConnection)
        - target_ip   = options.target_ip (actual connect address, passed as second arg)

    For IP targets:   target_name == target_ip (same value for both args)
    For FQDN targets: target_name = FQDN, target_ip = IP resolved via -ns nameserver
                      If no -ns given, target_ip = target_name (let OS/impacket resolve)
    """
    target_type, target_data = target

    target_name = target_data  # always the hostname/FQDN/IP as given
    target_ip = None

    if target_type.lower() in ["ip", "ipv4", "ipv6"]:
        # IP target: connect directly, no DNS needed
        target_ip = target_data

    elif target_type.lower() in ["fqdn"]:
        if options.nameserver is not None:
            # -ns specified: resolve via TCP DNS through proxychains
            target_ip = dns_resolve_tcp(options, target_data)
            if target_ip is not None:
                if options.debug:
                    lock.acquire()
                    print("[+] Resolved '%s' to %s via nameserver %s" % (target_name, target_ip, options.nameserver))
                    lock.release()
            else:
                if options.debug:
                    lock.acquire()
                    print("[!] Could not resolve '%s' via nameserver %s" % (target_name, options.nameserver))
                    lock.release()
                return
        else:
            # No -ns: mirror smbclient.py — pass FQDN directly to SMBConnection,
            # let impacket/OS handle resolution (works when DNS is reachable natively)
            target_ip = target_name

    if target_ip is not None:
        if is_port_open(target_ip, 445):
            try:
                # Pass both target_name (for NBT/Kerberos) and target_ip (for actual TCP connect)
                # This is exactly how impacket's smbclient.py does it:
                #   smbClient = SMBConnection(address, options.target_ip, sess_port=int(options.port))
                smbClient = init_smb_session(
                    options,
                    target_name,   # address  — hostname for NBT name negotiation / Kerberos SPN
                    target_ip,     # target_ip — actual IP to TCP connect to
                    domain,
                    username,
                    password,
                    lmhash,
...
def dns_resolve_tcp(options, target_name):
    """
    Resolves a hostname using TCP DNS only — proxychains compatible.
    Only called when -ns/--nameserver is explicitly provided.
    Mirrors the explicit nameserver pattern without any UDP fallback.
    """
    dns_resolver = dns.resolver.Resolver()
...
def parseArgs():
    ...
    parser.add_argument("-ns", "--nameserver", dest="nameserver", default=None, required=False,
                        help="IP of the DNS server to use for TCP DNS resolution (proxychains compatible). "
                             "If omitted, hostnames are passed directly to SMBConnection (impacket smbclient.py style).")
...
def print_results(options, shareData):
    try:
        ...
        if shareData["share"]["name"].endswith('$') and options.ignore_hidden_shares:
            do_print_results = False
        if ("STYPE_PRINTQ" in shareData["share"]["type"]["stype_flags"]) and options.ignore_print_queues:
            do_print_results = False
...
def load_targets(options):
    ...
    # Loading targets from specific LDAP query
    if not options.no_ldap:
        if options.auth_dc_ip is not None and options.auth_user is not None and (options.auth_password is not None or options.auth_hashes is not None) and options.target_ldap_query is not None:
            if options.debug:
                print("[debug] Loading targets from specific LDAP query '%s'" % options.target_ldap_query)
    ...
    if options.targets_urls_file is not None:
        if os.path.exists(options.targets_urls_file):
            ...
            print("[!] Could not open targets urls file '%s'" % options.targets_urls_file)
